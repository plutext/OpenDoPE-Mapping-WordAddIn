...
namespace XmlMappingTaskPane.Controls
{
    public partial class ControlTreeView : Controls.ControlBase
    {
        ...
        public ControlTreeView()
        {
            ...
            try
            {
                Options = (int)Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Office\XML Mapping Task Pane").GetValue("Options");
            }
        ...
        private void treeView_ItemDrag(object sender, ItemDragEventArgs e)
        {
            ...
            if (!IsLeafNode(tn) || ((XmlNode)tn.Tag).NodeType == XmlNodeType.Text && !IsLeafNode(tn.Parent) || ((XmlNode)tn.Tag).NodeType == XmlNodeType.ProcessingInstruction || ((XmlNode)tn.Tag).NodeType == XmlNodeType.Comment)
                return;
            ...
            if (OwnerDocument.Schemas.Count > 0)
            {
                switch (Utilities.CheckNodeType((XmlNode)tn.Tag))
                {
                    case Utilities.MappingType.Date:
                        strHTML = ClipboardUtilities.GenerateClipboardHTML(strXPath, Utilities.GetPrefixMappings(xmlnsMgr), CurrentPart.Id, Utilities.MappingType.Date);
                        break;
                    case Utilities.MappingType.DropDown:
                        strHTML = ClipboardUtilities.GenerateClipboardHTML(strXPath, Utilities.GetPrefixMappings(xmlnsMgr), CurrentPart.Id, Utilities.MappingType.DropDown);
                        break;
                    case Utilities.MappingType.Picture:
                        strHTML = ClipboardUtilities.GenerateClipboardHTML(strXPath, Utilities.GetPrefixMappings(xmlnsMgr), CurrentPart.Id, Utilities.MappingType.Picture);
                        break;
                    default:
                        strHTML = ClipboardUtilities.GenerateClipboardHTML(strXPath, Utilities.GetPrefixMappings(xmlnsMgr), CurrentPart.Id, Utilities.MappingType.Text);
                        break;
                }
            }
            else
        ...
        {
            if (((XmlNode)treeView.SelectedNode.Tag).NodeType == XmlNodeType.Text)
            {
        ...
        private void SetDefaultMappingCheckboxState()
        {
            ...
            if (CurrentDocument.Application.Selection.ParentContentControl != null
            ...
            else if (CurrentDocument.Application.Selection.ContentControls.Count == 1)
            {
                object objOne = 1;
        ...
        private void mapToSelectedControlToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    object objOne = 1;
                    CurrentDocument.Application.Selection.ContentControls.get_Item(ref objOne).XMLMapping.SetMappingByNode(Utilities.MxnFromTn(treeView.SelectedNode, CurrentPart, true));
                }
                else if (CurrentDocument.Application.Selection.ParentContentControl != null)
                {
                    CurrentDocument.Application.Selection.ParentContentControl.XMLMapping.SetMappingByNode(Utilities.MxnFromTn(treeView.SelectedNode, CurrentPart, true));
                }
        ...
        private void CreateMappedControl(Word.WdContentControlType CCType)
        {
            ...
            if (((XmlNode)tnSelected.Tag).NodeType == XmlNodeType.Text)
            {
                tnSelected = tnSelected.Parent;
            }
            Word.ContentControl cc = CurrentDocument.Application.Selection.ContentControls.Add(CCType, ref missing);
            cc.XMLMapping.SetMappingByNode(Utilities.MxnFromTn(tnSelected, CurrentPart, true));
        }
        ...
        private static bool IsLeafNode(TreeNode tn)
        {
